    ...
    try:
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                max_output_tokens=5000, temperature=0.3
            ),
        )
...
def main():
    ...
    # Read the Excel file with questions, specifying the sheet name Tugas_Akhir\pertanyaan\PENELITIAN_TA_1.xlsx
    df = pd.read_excel("Tugas_Akhir/pertanyaan/PENELITIAN_TA_3.xlsx", sheet_name="Pertanyaan_sederhana")
    ...
    with token_tracker:
        ...
        for index, row in df.iterrows():
            ...
            entry = {"Pertanyaan": question}

            # Fungsi util untuk safe query
            def safe_query(mode):
                try:
                    token_tracker.reset()
                    ans = rag.query(question, param=QueryParam(mode=mode, 
                                                               response_type="Single Paragraph", 
                                                               top_k=5, chunk_top_k=5,
                                                               max_entity_tokens=2500, max_relation_tokens=2500,
                                                               max_total_tokens=12000, enable_rerank=True))
                    usage = token_tracker.get_usage()
                    prompt = token_tracker.get_prompt()
                    return ans, usage, prompt
                except Exception as e:
                    # Tangkap error, isi dengan pesan
            ...
            # Naive
            naive_ans, naive_usage, naive_prompt = safe_query("naive")
            entry.update({
                "Jawaban Naive": naive_ans,
                "Naive Token Count": naive_usage["total_tokens"],
                "Naive Prompt": naive_prompt,
                "Naive Prompt Token": naive_usage["prompt_tokens"],
                "Naive Completion Token": naive_usage["completion_tokens"]
            })

            # Local
            local_ans, local_usage, local_prompt = safe_query("local")
            entry.update({
                "Jawaban Local": local_ans,
                "Local Token Count": local_usage["total_tokens"],
                "Local Prompt": local_prompt,
                "Local Prompt Token": local_usage["prompt_tokens"],
                "Local Completion Token": local_usage["completion_tokens"]
            })

            # Global
            global_ans, global_usage, global_prompt = safe_query("global")
            entry.update({
                "Jawaban Global": global_ans,
                "Global Token Count": global_usage["total_tokens"],
                "Global Prompt": global_prompt,
                "Global Prompt Token": global_usage["prompt_tokens"],
                "Global Completion Token": global_usage["completion_tokens"]
            })

            # Hybrid
            hybrid_ans, hybrid_usage, hybrid_prompt = safe_query("hybrid")
            entry.update({
                "Jawaban Hybrid": hybrid_ans,
                "Hybrid Token Count": hybrid_usage["total_tokens"],
                "Hybrid Prompt": hybrid_prompt,
                "Hybrid Prompt Token": hybrid_usage["prompt_tokens"],
                "Hybrid Completion Token": hybrid_usage["completion_tokens"]
            })
    ...
    try :
        result_df.to_csv("./Tugas_Akhir/results/gemini_3_1.csv", index=False, quoting=csv.QUOTE_ALL)  # Menggunakan QUOTE_ALL untuk mengutip semua kolom
        result_df.to_excel("./Tugas_Akhir/results/gemini_3_1.xlsx", index=False)  # Uncomment to save as Excel
